import { PNodeInfo, PNodeStats } from "@/types/pnode";
...
export function analyzeNetwork(
  ...
  const healthScore = Math.round(
    (healthCounts.healthy / pnodes.length) * 60 + 
      (1 - outdatedNodes.length / pnodes.length) * 30 + 
      (healthCounts.degraded / pnodes.length) * 10 
  );
